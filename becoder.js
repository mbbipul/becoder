...
firestore.collection('problems').where('ProblemOwnerID', '==', getCurrentUserId())
      .onSnapshot(function(snapshot) {
          snapshot.docChanges().forEach(function(change) {
              if (change.type === "added") {
                console.log("Modified city: ", change.doc.data());
                ...
                var tableRowDataPP = document.createElement("td");
                var tableRowDataPS = document.createElement("td");
                var tableRowDataPOI = document.createElement("td");

                tableRowHeader.setAttribute("scope","row");

                var contestCount = document.createTextNode(1);
                var contestNameText = document.createTextNode(data.ProblemName);
                var contestOwnerText = document.createTextNode(data.ProblemOwner);
                var problemSolved = document.createTextNode(data.Solved);
                var ProblemPartiCipants = document.createTextNode(data.Participants);
                var ProblemOwnerID = document.createTextNode(data.ProblemOwnerID);


                tableRowHeader.appendChild(contestCount);
                tableRowDataPN.appendChild(contestNameText);
                tableRowDataPO.appendChild(contestOwnerText);
                tableRowDataPP.appendChild(ProblemPartiCipants);
                tableRowDataPS.appendChild(problemSolved);
                tableRowDataPOI.appendChild(ProblemOwnerID);

                tableRow.appendChild(tableRowHeader);
                tableRow.appendChild(tableRowDataPN);
                tableRow.appendChild(tableRowDataPO);
                tableRow.appendChild(tableRowDataPP);
                tableRow.appendChild(tableRowDataPS);
                tableRow.appendChild(tableRowDataPOI);
